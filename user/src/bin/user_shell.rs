...
impl ProcessArguments {
    pub fn new(command: &str) -> Self {
        // 现在 line 还可能包含一些命令行参数，只有最开头的一个才是要执行的应用名。因此我们要做的第一件事情就是将 line 用空格进行分割
        let args: Vec<_> = command.split(' ').collect();
        // 因为 line 是我们输入得到的，中间本来就没有 \0 。由于在向内核传入字符串的时候，我们只能传入字符串的起始地址，因此我们必须保证其结尾为 \0 。
        // 从而我们用 args_copy 将 args 中的字符串拷贝一份到堆上并在末尾手动加入 \0 。这样就可以安心的将 args_copy 中的字符串传入内核了
        let mut args_copy: Vec<String> = args
            ...
            .collect();
        // 在分割命令行参数的时候，我们要检查是否存在通过 < 或 > 进行输入输出重定向的情况，
        // 如果存在的话则需要将它们从命令行参数中移除，并记录匹配到的输入文件名或输出文件名到字符串 input 或 output 中
        // 这里假设输入shell程序的命令一定合法：即 < 或 > 最多只会出现一次，且后面总是会有一个参数作为重定向到的文件
        // redirect input
        ...
        // 向量 args_addr 中的每个元素都代表一个命令行参数字符串的起始地址。由于我们要传递给内核的是这个向量的起始地址，
        // 为了让内核能够获取到命令行参数的个数，我们需要在 args_addr 的末尾放入一个 0 ，这样内核看到它的时候就能知道命令行参数已经获取完毕了
                        ...
                        for (i, process_argument) in process_arguments_list.iter().enumerate() {
                            ...
                            if pid == 0 {
                                ...
                                if !input.is_empty() {
                                    ...
                                    close(0);
                                    // 通过 dup 来分配一个新的文件描述符来访问 input_fd 对应的输入文件
                                    // 这里用到了文件描述符分配的重要性质：即必定分配可用描述符中编号最小的一个。由于我们刚刚关闭了描述符 0 ，那么在 dup 的时候一定会将它分配出去，于是现在应用进程的文件描述符 0 就对应到输入文件了

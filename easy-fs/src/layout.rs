...
pub struct SuperBlock {
    ...
    magic: u32,
    // total_block 给出文件系统的总块数(这并不等同于所在磁盘的总块数，因为文件系统很可能并没有占据整个磁盘)
...
pub struct DiskInode {
    // size 表示文件/目录内容的字节数
    pub size: u32,
    // direct/indirect1/indirect2 都是存储文件内容/目录内容的数据块的索引
    // 块的编号用一个 u32 存储。索引方式分成直接索引和间接索引两种
    pub direct: [u32; INODE_DIRECT_COUNT],
    // 一级间接索引 indirect1 。它指向一个一级索引块，这个块也位于磁盘布局的数据块区域中。这个一级索引块中的每个 u32 都用来指向数据块区域中一个保存该文件内容的数据块
    // 因此，最多能够索引512/4=128个数据块，对应 64KiB 的内容
    pub indirect1: u32,
    pub indirect2: u32,
...
impl DiskInode {
    // initialize 方法可以初始化一个 DiskInode 为一个文件或目录
    /// Initialize a disk inode, as well as all direct inodes under it
    /// indirect1 and indirect2 block are allocated only when they are needed
    pub fn initialize(&mut self, type_: DiskInodeType) {
        self.size = 0;
        // indirect1/2 均被初始化为 0 。因为最开始文件内容的大小为 0 字节，并不会用到一级/二级索引。为了节约空间，内核会按需分配一级/二级索引块。直接索引 direct 也被清零
    ...
    // blocks_num_needed 可以计算将一个 DiskInode 的 size 扩容到 new_size 需要额外多少个数据和索引块。这只需要调用两次 total_blocks 作差即可
    ...
    // write_at 不会出现失败的情况；只要 Inode 管理的数据块的大小足够，传入的整个缓冲区的数据都必定会被写入到文件中。
...
// 每个目录项都是一个二元组，二元组的首个元素是目录下面的一个文件（或子目录）的文件名（或目录名），另一个元素则是文件（或子目录）所在的索引节点编号
// 目录项 DirEntry 最大允许保存长度为 27 的文件/目录名（数组 name 中最末的一个字节留给 \0 ），且它自身占据空间 32 字节
/// A directory entry
#[repr(C)]
pub struct DirEntry {
...
impl DirEntry {
    ...
    pub fn name(&self) -> &str {
        let len = (0usize..).find(|i: &usize| self.name[*i] == 0).unwrap();
